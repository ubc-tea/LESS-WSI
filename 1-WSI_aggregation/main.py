...
import models
from models.csmil import DeepAttnMIL_Surv
import utils
# if usewandb:
import wandb
def setup_seed(seed):
...
def main(args,nth_fold):
    utils.init_distributed_mode(args)
    print(args)
    device = torch.device(args.device)
    watermark = "bz{}_lr{}_seed{}_fold{}_sched{}_ep{}_lr".format(args.batch_size, args.lr,args.seed,args.nth_fold,args.sched,args.epochs,args.lr)
    wandb.init(project="CrossViT_rebuttal_FNAC1",
               name=watermark,
               entity = 'beidiz',
               group='VPU+CrossViT')
    ...
    dataset_train1,dataset_train2, args.nb_classes = build_dataset(is_train=True, args=args,nth_fold=nth_fold)
    ...
    dataset_val1,dataset_val2, _ = build_dataset(is_train=False, args=args,nth_fold=nth_fold)
    ...
    data_loader_val1 = torch.utils.data.DataLoader(
        ...
        dataset_val1, batch_size=args.batch_size,
    ...
    data_loader_val2 = torch.utils.data.DataLoader(
        # dataset_val, sampler=val_sampler, batch_size=int(1.5 * args.batch_size),
        dataset_val2, batch_size=args.batch_size,
        # shuffle=False, num_workers=args.num_workers,
        shuffle=False, num_workers=args.num_workers,
        pin_memory=args.pin_mem, drop_last=False
    )
    print(f'Number of training data: {len(dataset_train1)}')
    print(f'Number of test data: {len(dataset_val1)}')
    print('Length of train loader',len(data_loader_train1))
    print('Length of validation loader',len(data_loader_val1))

    print(f"Creating model: {args.model}")
    if args.model == 'csmil':
        model = DeepAttnMIL_Surv()
    else:
        model = create_model(
            args.model,
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val1,data_loader_val2, model, device, num_tasks, distributed=True, amp=args.amp)
        print(f"Accuracy of the network on the {len(dataset_val1)} test images: {test_stats['acc1']:.2f}%")
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, criterion, data_loader_train1,data_loader_train2,
            optimizer, device, epoch, loss_scaler,
            args.clip_grad, num_tasks, True,
            amp=args.amp,
            finetune=args.finetune
        )

        lr_scheduler.step(epoch-1) # step cosine scheduling
        ...
        wandb.log({'epoch': epoch, 'train_loss': train_stats["loss"], 'val_loss': test_stats["loss"], "val_acc": test_stats["acc1"],
               ...
               'Acc_low':Evaluation[0],'Acc_high':Evaluation[1],'Acc_benign':Evaluation[2],'Acc_atypical':Evaluation[3],'Acc_suspicious':Evaluation[4],'Acc_cancer':Evaluation[5]})
