...
mean =[0.5327, 0.6170, 0.7039]
std =[0.1500, 0.2103, 0.1681]
...
class EmbedDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if filename.split('/')[-2] == 'B':
            label = [int(1)]
        if filename.split('/')[-2] == 'M':
            label = [int(0)]
...
class urine_labeled_dataset(Dataset):
    ...
    def __init__(self, dataset_path='', target_transform=None, data_class = None):
        ...
        train_data_path = dataset_path
        if data_class == 'p':
            self.imagenet = FilterableImageFolder(root=train_data_path, valid_classes=['B'])
        elif data_class == 'u':
            self.imagenet = FilterableImageFolder(root=train_data_path, valid_classes=['M'])
        else:
            self.imagenet = datasets.ImageFolder(root=train_data_path)
...
class UrineSlideDataset(Dataset):
    ...
    def __getitem__(self, index):
        image_tensor = self.transform(self.imagenet[index][0])
        # label_tensor = int(self.imagenet[index][1])
        if self.dataset_path.split('/')[-2] == 'M':
            label_tensor = [int(0)]
        if self.dataset_path.split('/')[-2] == 'B':
            label_tensor = [int(1)]
...
def get_urine_data(positive_label_list,nth_fold):
    ...
    if nth_fold > 0:
        train_labeled_dataset = urine_labeled_dataset('/bigdata/projects/beidi/FANC/256_'+str(nth_fold)+'/train', data_class='p', target_transform=target_transform)
        train_unlabeled_dataset = urine_labeled_dataset('/bigdata/projects/beidi/FANC/256_'+str(nth_fold)+'/train', data_class ='u', target_transform=target_transform)
        val_unlabeled_dataset = urine_labeled_dataset('/bigdata/projects/beidi/FANC/256_'+str(nth_fold)+'/test', data_class ='u', target_transform=target_transform)
        val_labeled_dataset = urine_labeled_dataset('/bigdata/projects/beidi/FANC/256_'+str(nth_fold)+'/test', data_class ='p',target_transform=target_transform)
        test_dataset = urine_labeled_dataset('/bigdata/projects/beidi/FANC/256_'+str(nth_fold)+'/test',target_transform=target_transform)
    else:
        train_labeled_dataset = urine_labeled_dataset('/bigdata/projects/beidi/FANC/256_1/train',data_class='p', target_transform=target_transform)
        train_unlabeled_dataset = urine_labeled_dataset('/bigdata/projects/beidi/FANC/256_1/train', data_class='u', target_transform=target_transform)
        val_unlabeled_dataset = urine_labeled_dataset('/bigdata/projects/beidi/FANC/256_1/test',data_class='u', target_transform=target_transform)
        val_labeled_dataset = urine_labeled_dataset('/bigdata/projects/beidi/FANC/256_1/test',data_class='p', target_transform=target_transform)
        test_dataset = urine_labeled_dataset('/bigdata/projects/beidi/FANC/256_1/test',target_transform=target_transform)
